...
| A feature `helpers/` imported by *another* feature, or growing domain/API/UI logic | local implementation detail leaking / becoming a layer in disguise | keep `helpers/` private to the feature (the hard rule); promote a helper to its named layer when the pain is real — a second consumer, or a reason-to-change that maps onto an existing layer (not on first touch) | `conventions.md` glossary |
